...
public class NvtMeterReadingServiceImpl implements NvtMeterReadingService {
    ...
    @Override
    public Optional<NvtMeterReading> findByRoomAndMonthAndYear(Long roomId, Integer month, Integer year) {
        return repo.findByRoomIdAndBillMonthAndBillYear(roomId, month, year);
    }
    ...
    public NvtMeterReading create(NvtMeterReading r) {
        ...
        if (r.getElectricOld() == null || r.getWaterOld() == null) {
            repo.findTopByRoomIdOrderByBillYearDescBillMonthDesc(r.getRoomId()).ifPresent(prev -> {
                if (r.getElectricOld() == null)
                    r.setElectricOld(prev.getElectricNew());
                if (r.getWaterOld() == null)
                    r.setWaterOld(prev.getWaterNew());
            });
            if (r.getElectricOld() == null)
                r.setElectricOld(0);
            if (r.getWaterOld() == null)
        ...
        if (r.getElectricNew() == null)
            r.setElectricNew(r.getElectricOld());
        if (r.getWaterNew() == null)
            r.setWaterNew(r.getWaterOld());
